...
if __name__ == "__main__":
    log_path = setup_logging(run_date = None)
    ...
    if cfg['mesh']['dim'] == 2 and cfg['terrain'].get('enabled', False):
        logger.info("Generating terrain-based velocity field...")
        h, vx, vy = make_terrain_and_velocity(cfg)
        mesh.set_terrain_and_velocity(h, vx, vy)
        logger.info(f"Terrain attached to mesh")
        logger.info(f"  Terrain shape: {h.shape}")
        logger.info(f"  Terrain range: [{h.min():.4f}, {h.max():.4f}]")
        v_mag = np.sqrt(vx ** 2 + vy ** 2)
        logger.info(f"  Velocity magnitude range: [{v_mag.min():.6f}, {v_mag.max():.6f}]")
        logger.info(f"  Max velocity magnitude: {v_mag.max():.6f}")
    else:
        logger.info(f"Terrain disabled (using constant velocity)")
        logger.info(f"  terrain.enabled = {cfg['terrain'].get('enabled')}")
        logger.info(f"  mesh.dim = {cfg['mesh']['dim']}")
